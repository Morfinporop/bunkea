...
interface GameContextType {
  ...
  // Host actions
  startGame: () => Promise<{ error?: string }>;
...
export function GameProvider({ children }: { children: React.ReactNode }) {
  ...
  const startGame = useCallback(async () => {
    const res = await emit<{ error?: string }>('start_game', { roomCode: room?.code });
    return res;
  }, [emit, room]);
  ...
  return (
    <GameContext.Provider value={{
      ...
      updateSettings, changeCatastrophe, reassignCards, restartGame, kickPlayer,
      vote, playerRevealCard,
